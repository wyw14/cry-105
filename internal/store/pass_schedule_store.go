...
func (s *WindowStore) SaveBoundary(window WindowRecord) error {
	s.mu.Lock()
	current := s.windows[window.PassID]
	current.PassID = window.PassID
	current.AOS = window.AOS
	current.Revision = window.Revision
	current.Updated = time.Now().UTC()
	s.windows[window.PassID] = current
